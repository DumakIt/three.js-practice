...
export default function example() {
  ...
  const setShape = (event) => {
    ...
    for (let i = 0; i < imagePanels.length; i++) {
      ...
      // 회전
      if (type === "random") {
        gsap.to(imagePanels[i].mesh.rotation, { duration: 2, x: 0, y: 0, z: 0 });
      } else {
        gsap.to(imagePanels[i].mesh.rotation, { duration: 2, x: imagePanels[i].sphereRotationX, y: imagePanels[i].sphereRotationY, z: imagePanels[i].sphereRotationZ });
      }
    }
  };
  ...
  randomBtn.style.cssText = "position: absolute; top: 20px; left: 20px;";
  randomBtn.innerHTML = "Random";
  btnWrapper.append(randomBtn);

  const sphereBtn = document.createElement("button");
  sphereBtn.dataset.type = "sphere";
  sphereBtn.style.cssText = "position: absolute; top: 50px; left: 20px;";
